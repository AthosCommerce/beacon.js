...
// Mock localStorage - shared across all test suites to avoid Object.defineProperty conflicts
...
for (const { isAthos, siteId } of [
	{ isAthos: false, siteId: 'abc123' },
	{ isAthos: true, siteId: 'athos0' },
]) {
	...
	describe(`Beacon ${isAthos ? 'Athos' : 'Searchspring'} Tests`, () => {
		let beacon: Beacon;
		const mockFetchApi = jest.fn().mockResolvedValue(Promise.resolve({ status: 200, json: () => Promise.resolve({}) }));
		const mockGlobals = { siteId };
		const storageArea = isAthos ? 'primary' : 'legacy';
		const preflightDomain = isAthos ? 'athoscommerce.net' : 'searchspring.io';
		const apiBasePath = isAthos ? 'analytics.athoscommerce.net' : 'analytics.searchspring.net';
		const mockConfig = {
			apis: {
				fetch: mockFetchApi,
			},
		...
		describe('Storage', () => {
			describe('Cart', () => {
				...
				it('favors primary key', () => {
					localStorageMock.setItem(STORAGE_KEYS.cartProducts.primary, JSON.stringify({ value: isAthos ? mockProducts : [] }));
					localStorageMock.setItem(STORAGE_KEYS.cartProducts.legacy, JSON.stringify({ value: isAthos ? [] : mockProducts }));
				...
				it('can set and get cart products', () => {
					...
					expect(global.document.cookie).toContain(`${STORAGE_KEYS.cartProducts[storageArea]}=${encodedCartCookie}`);

					// localStorage contains cart data (both primary and legacy keys)
					expect(localStorageMock.setItem).toHaveBeenCalled();
					expect(localStorageMock.getItem(STORAGE_KEYS.cartProducts[storageArea])).toBe(JSON.stringify({ value: mockProducts }));
					...
					expect(global.document.cookie).toContain(`${STORAGE_KEYS.cartProducts[storageArea]}=`);
					expect(localStorageMock.getItem(STORAGE_KEYS.cartProducts[storageArea])).toBe(JSON.stringify({ value: [] }));
				});
			});
			describe('Methods', () => {
				...
				it('can getPageLoadId', async () => {
					...
					const stored = localStorageMock.getItem(STORAGE_KEYS.pageLoadId[storageArea]) || '{}';
				...
				it('can getPageLoadId from storage', async () => {
					...
					localStorageMock.setItem(STORAGE_KEYS.pageLoadId[storageArea], JSON.stringify({ value: stored }));
					...
					const stored2 = localStorageMock.getItem(STORAGE_KEYS.pageLoadId[storageArea]) || '{}';
		...
		describe(`${preflightDomain} domain tests`, () => {
			it(`can switch siteIds to ${preflightDomain}`, async () => {
		...
		describe('Events', () => {
			...
			describe('Shopper Login', () => {
				it('can process login event', async () => {
					...
					expect(mockFetchApi).toHaveBeenCalledWith(expect.stringContaining(`${apiBasePath}/beacon/v2`), fetchPayloadAssertion);
				});
		...
		describe('sendPreflight', () => {
			it('can sendPreflight via POST', async () => {
				...
				expect(mockFetchApi).toHaveBeenCalledWith(`https://${mockGlobals.siteId}.a.${preflightDomain}/v1/preflight`, {
					method: 'POST',
					headers: {
			...
			it(`can sendPreflight via POST to ${preflightDomain}`, async () => {
				// only add 1 product to be under threshold and still generate GET request
				const items = [{ uid: 'uid123', sku: 'sku123', parentId: 'parentId123', qty: 1, price: 10.99 }];
